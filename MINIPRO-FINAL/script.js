...
{ id: 1, name: 'Alex Johnson', username: 'alexj', password: 'password123', position: 'Store Manager', status: 'Active', sales: 5240, rating: 4.8, attendance: 98, ratingCount: 24, totalRating: 115.2 },
{ id: 2, name: 'Sarah Williams', username: 'sarahw', password: 'password123', position: 'Sales Associate', status: 'Active', sales: 3890, rating: 4.2, attendance: 95, ratingCount: 18, totalRating: 75.6 },
{ id: 3, name: 'Michael Brown', username: 'michaelb', password: 'password123', position: 'Inventory Clerk', status: 'Active', sales: 2150, rating: 3.6, attendance: 92, ratingCount: 12, totalRating: 43.2 },
{ id: 4, name: 'Emily Davis', username: 'emilyd', password: 'password123', position: 'Cashier', status: 'Inactive', sales: 1890, rating: 4.1, attendance: 97, ratingCount: 15, totalRating: 61.5 }
];
...
// Initialize store settings from localStorage
...
updateBillHeader(); // Update bill header with new settings
updateSupportInfo(); // Update customer support info with new settings
}
...
<div class="task-priority badge ${getPriorityBadgeClass(task.priority)}">${task.priority}</div>
</div>
<div class="task-meta">
...
<span class="badge ${getStatusBadgeClass(task.status)}">${task.status}</span>
 ${task.status !== 'Completed' ? `<button class="btn btn-small" onclick="updateTaskStatus(${task.id}, 'Completed')">Mark Complete</button>` : ''}
</div>
`;
...
// Update staff task list
const staffTaskList = document.getElementById('staffTaskList');
if (staffTaskList) {
staffTaskList.innerHTML = '';

const staffTasks = tasks.filter(task => task.assignedTo === currentUser.id);

if (staffTasks.length === 0) {
staffTaskList.innerHTML = '<p style="text-align: center; padding: 2rem;">No tasks assigned to you.</p>';
} else {
...
<div class="task-actions">
<span class="badge ${getStatusBadgeClass(task.status)}">${task.status}</span>
 ${task.status === 'Pending' ? `<button class="btn btn-small" onclick="startStaffTask(${task.id})">Start Task</button>` : ''}
 ${task.status === 'In Progress' ? `<button class="btn btn-small" onclick="completeStaffTask(${task.id})">Complete Task</button>` : ''}
</div>
`;
staffTaskList.appendChild(taskItem);
});
}
}

// Update pending tasks count
const pendingTasks = tasks.filter(task => task.status === 'Pending' && task.assignedTo === currentUser.id);
document.getElementById('pendingTasksCount').textContent = pendingTasks.length;
}

// Update staff ratings in customer dashboard
function updateStaffRatings() {
// Update staff cards in customer dashboard
const staffCards = document.querySelectorAll('.staff-card');
staffCards.forEach((card, index) => {
if (index < staff.length) {
const staffMember = staff[index];
const ratingElement = card.querySelector('.staff-rating span');
if (ratingElement) {
ratingElement.textContent = `${staffMember.rating.toFixed(1)} (${staffMember.ratingCount} reviews)`;
}

const starsElement = card.querySelector('.stars');
if (starsElement) {
starsElement.innerHTML = generateStars(staffMember.rating);
}
}
});
}

// Update support info in customer dashboard
function updateSupportInfo() {
document.getElementById('supportStoreName').textContent = storeSettings.name;
document.getElementById('supportStoreEmail').textContent = storeSettings.email;
document.getElementById('supportStorePhone').textContent = storeSettings.phone;
document.getElementById('supportStoreAddress').textContent = storeSettings.address;
}
...
document.getElementById('billStoreName').textContent = storeSettings.name;
document.getElementById('billStoreAddress').textContent = storeSettings.address;
document.getElementById('billStorePhone').textContent = storeSettings.phone;
}
...
currentRole = 'admin';
currentUser = { name: 'Admin', username: 'admin', id: 0 };
showDashboard('admin');
} else {
...
if (staffMember && staffMember.password && password === staffMember.password) {
currentRole = 'staff';
...
const password = document.getElementById('staffPasswordReg').value;

// Only username and password are required
const username = document.getElementById('staffUsernameReg').value || 'staff_' + Date.now();
...
const newStaff = {
id: Date.now(),
name: name || 'Unnamed Staff', // Default if not provided
username,
password, // Use the provided password
position: position || 'Staff', // Default if not provided
status: 'Active',
sales: 0,
rating: 0,
attendance: 0,
...
<button class="btn btn-small btn-danger" onclick="deleteStaff(${staffMember.id})">Delete</button>
</td>
`;
staffTableBody.appendChild(row);
});
...
function loadTaskAssignOptions() {
const taskAssignedTo = document.getElementById('taskAssignedTo');
if (taskAssignedTo) {
taskAssignedTo.innerHTML = '<option value="">Select staff member</option>';

staff.forEach(staffMember => {
if (staffMember.status === 'Active') {
const option = document.createElement('option');
option.value = staffMember.id;
option.textContent = `${staffMember.name} (${staffMember.position})`;
...
function startStaffTask(taskId) {
updateTaskStatus(taskId, 'In Progress');
}

function completeStaffTask(taskId) {
...
// Modal functions
function openModal(modalId, staffName = '', staffId = null) {
...
const staffMember = staff.find(s => s.id === currentStaffId);
if (staffMember) {
// Update staff member's rating
staffMember.totalRating += selectedStaffRating;
staffMember.ratingCount += 1;
staffMember.rating = staffMember.totalRating / staffMember.ratingCount;
